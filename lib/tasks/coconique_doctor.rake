# lib/tasks/coconique_doctor.rake

namespace :coconique do
  desc "Check runtime configuration for local/staging/production"
  task doctor: :environment do
    result = CoconiqueDoctor.new.run
    puts result[:lines].join("\n")
    abort "coconique:doctor failed" unless result[:ok]
  end
end

class CoconiqueDoctor
